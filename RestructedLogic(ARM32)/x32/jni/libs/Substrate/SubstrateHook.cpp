...
// x=0; while IFS= read -r line; do if [[ ${#line} -ne 0 && $line == +([^\;]): ]]; then x=2; elif [[
// $line == ' +'* && $x -ne 0 ]]; then ((--x)); echo "$x${line}"; fi; done <WebCore.asm >WebCore.pc
// grep pc WebCore.pc | cut -c 40- | sed -Ee 's/^ldr *(ip|r[0-9]*),\[pc,\#0x[0-9a-f]*\].*/ ldr
// r*,[pc,#*]/;s/^add *r[0-9]*,pc,r[0-9]*.*/ add r*, pc,r*/;s/^(st|ld)r
// *r([0-9]*),\[pc,r([0-9]*)\].*/ \1r r\2,[pc,r\3]/;s/^fld(s|d)
// *(s|d)[0-9]*,\[pc,#0x[0-9a-f]*].*/fld\1 \2*,[pc,#*]/' | sort | uniq -c | sort -n

#include "SubstrateARM.hpp"

#define T$Label(l, r) (((r) - (l)) * 2 - 4 + ((l) % 2 == 0 ? 0 : 2))

#define T$pop_$r0$ 0xbc01  // pop {r0}
#define T$b(im) /* b im */ (0xde00 | (im & 0xff))
#define T$blx(rm) /* blx rm */ (0x4780 | (rm << 3))
#define T$bx(rm) /* bx rm */ (0x4700 | (rm << 3))
#define T$nop /* nop */ (0x46c0)

#define T$add_rd_rm(rd, rm) /* add rd, rm */                                            \
  (0x4400 | (((rd) & 0x8) >> 3 << 7) | (((rm) & 0x8) >> 3 << 6) | (((rm) & 0x7) << 3) | \
   ((rd) & 0x7))
#define T$push_r(r) /* push r... */ (0xb400 | (((r) & (1 << A$lr)) >> A$lr << 8) | ((r) & 0xff))
#define T$pop_r(r) /* pop r... */ (0xbc00 | (((r) & (1 << A$pc)) >> A$pc << 8) | ((r) & 0xff))
#define T$mov_rd_rm(rd, rm) /* mov rd, rm */                                            \
  (0x4600 | (((rd) & 0x8) >> 3 << 7) | (((rm) & 0x8) >> 3 << 6) | (((rm) & 0x7) << 3) | \
   ((rd) & 0x7))
#define T$ldr_rd_$rn_im_4$(rd, rn, im) /* ldr rd, [rn, #im * 4] */ \
  (0x6800 | (((im) & 0x1f) << 6) | ((rn) << 3) | (rd))
#define T$ldr_rd_$pc_im_4$(rd, im) /* ldr rd, [PC, #im * 4] */ \
  (0x4800 | ((rd) << 8) | ((im) & 0xff))
#define T$cmp_rn_$im(rn, im) /* cmp rn, #im */ (0x2000 | ((rn) << 8) | ((im) & 0xff))
#define T$it$_cd(cd, ms) /* it<ms>, cd */ (0xbf00 | ((cd) << 4) | (ms))
...
#define T2$ldr_rt_$rn_im$(rt, rn, im) /* ldr rt, [rn, #im] */ (((rt) << 12) | labs(im))

#define T1$mrs_rd_apsr(rd) /* mrs rd, apsr */ (0xf3ef)
#define T2$mrs_rd_apsr(rd) /* mrs rd, apsr */ (0x8000 | ((rd) << 8))

#define T1$msr_apsr_nzcvqg_rn(rn) /* msr apsr, rn */ (0xf380 | (rn))
#define T2$msr_apsr_nzcvqg_rn(rn) /* msr apsr, rn */ (0x8c00)
...
static size_t SubstrateHookFunctionThumb(SubstrateProcessRef process, void *symbol, void *replace,
  ...
  if ((align == 0 || area[0] == T$nop) && thumb[0] == T$bx(A$pc) && thumb[1] == T$nop &&
  ...
  if (result != NULL) {
    ...
    uint16_t *buffer(reinterpret_cast<uint16_t *>(
        mmap(NULL, length, PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0)));
...
static size_t SubstrateHookFunctionARM(SubstrateProcessRef process, void *symbol, void *replace,
                                       void **result) {
  if (symbol == NULL)
    return 0;
...
static size_t SubstrateHookFunction(SubstrateProcessRef process, void *symbol, void *replace,
                                    void **result) {
  if (MSDebug)
  ...
  if ((reinterpret_cast<uintptr_t>(symbol) & 0x1) == 0)
    ...
    return SubstrateHookFunctionThumb(
        process, reinterpret_cast<void *>(reinterpret_cast<uintptr_t>(symbol) & ~0x1), replace,
        result);
}
#endif
...
static void SubstrateHookFunction(SubstrateProcessRef process, void *symbol, void *replace,
                                  void **result) {
  if (MSDebug)
    ...
    for (size_t offset(0), width; offset != used; offset += width) {
      ...
      if ((decode.modrm & 0xc7) == 0x05) {
        if (decode.opcode == 0x8b) {
          ...
          MSLog(MSLogLevelError, "MS:Error: Unknown RIP-Relative (%.2x %.2x)", decode.opcode,
                decode.opcode2);
          continue;
        }
      } else

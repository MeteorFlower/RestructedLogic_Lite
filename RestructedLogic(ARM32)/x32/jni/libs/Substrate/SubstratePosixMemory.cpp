...
struct __SubstrateMemory {
  ...
  __SubstrateMemory(void *address, size_t width) : address_(address), width_(width) {}
};

extern "C" SubstrateMemoryRef SubstrateMemoryCreate(SubstrateAllocatorRef allocator,
                                                    SubstrateProcessRef process, void *data,
                                                    size_t size) {
  if (allocator != NULL) {
  ...
  long page(sysconf(_SC_PAGESIZE));  // Portable applications should employ sysconf(_SC_PAGESIZE)
